...
class MainWindow(QMainWindow):
    ...
    def create_center_panel(self):
        ...
        # --- REMOVED: 移除会导致比例失调的 setScaledContents ---
        # self.preview_label.setScaledContents(True) 
        
        # --- KEPT: 保留解决布局问题的 setSizePolicy ---
        self.preview_label.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Ignored)

        layout.addWidget(self.preview_label)
        return center_frame

    # ... create_right_panel 及其槽函数保持不变 ...
    def create_right_panel(self):
        # ... 此方法保持不变 ...
        right_frame = QFrame(); right_frame.setFrameShape(QFrame.Shape.StyledPanel)
        main_layout = QVBoxLayout(right_frame)
        settings_label = QLabel("水印设置"); settings_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        self.font_combo = QFontComboBox(); self.font_combo.setCurrentFont(self.watermark_font); self.font_combo.currentFontChanged.connect(self.on_font_changed)
        form_layout.addRow("字体:", self.font_combo)
        self.font_size_spinbox = QSpinBox(); self.font_size_spinbox.setRange(8, 200); self.font_size_spinbox.setValue(self.watermark_font.pointSize()); self.font_size_spinbox.valueChanged.connect(self.on_font_size_changed)
        ...
        color_layout.addWidget(self.btn_color); color_layout.addWidget(self.color_preview)
        form_layout.addRow("颜色:", color_layout)
        self.opacity_slider = QSlider(Qt.Orientation.Horizontal); self.opacity_slider.setRange(0, 255); self.opacity_slider.setValue(self.watermark_color.alpha()); self.opacity_slider.valueChanged.connect(self.on_opacity_changed)
    ...
    def on_color_clicked(self):
        ...
        if color.isValid():
            alpha = self.watermark_color.alpha(); color.setAlpha(alpha)
            self.watermark_color = color; self.update_color_preview(); self.update_display()
    ...
    # --- NEW: 恢复 resizeEvent 以确保窗口缩放时图片也同步缩放 ---
